...
inline static Expression::Operator tokenToOperator(const std::vector<Token>& tokens, const int tokenIndex) {
	...
	switch (t) {
		...
		case Token::Type::Sqrt: return Expression::Operator::Sqrt;
		default: {
			assert(false);
...
inline static float evaluateOperator(const Expression::Operator op, const float operand) {
	switch (op) {
		...
		case Expression::Operator::Sqrt: return sqrtf(operand);
...
inline static Token::Type parseOperator(const char* start, int& len) {
	...
	} else if (!strncmp(start, "sqrt", 4)) {
		len = 4;
		return Token::Type::Sqrt;
	}
	len = 0;
...
EC::ErrorCode Expression::init(const char* expression) {
	...
	// case we add the operator on the operator stack and proceed.
	...
	for (int i = 0; i < tokensSize; ++i) {
		...
		if (t.isOperator()) {
			...
			if (op == Expression::Operator::Sin || op == Expression::Operator::Cos || op == Expression::Operator::Sqrt) {
